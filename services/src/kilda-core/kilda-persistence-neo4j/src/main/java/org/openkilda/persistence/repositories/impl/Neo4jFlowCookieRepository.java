...
public class Neo4jFlowCookieRepository extends Neo4jGenericRepository<FlowCookie> implements FlowCookieRepository {
    ...
    public Optional<FlowCookie> findById(Cookie cookie) {
        ...
        Collection<FlowCookie> cookies = getSession().loadAll(getEntityType(), cookieFilter,
                DEPTH_LOAD_ENTITY);
        if (cookies.size() > 1) {
    ...
    public Optional<FlowCookie> findByPathId(PathId pathId) {
        ...
        Collection<FlowCookie> cookies = getSession().loadAll(getEntityType(), pathIdFilter,
                DEPTH_LOAD_ENTITY);
        if (cookies.size() > 1) {
